...
class StrategyClosingOnHighs(BaseStrategy):
    ...
    def _process_data(self, data) -> None:
        ...
        if len(data) == i_last_candle - 1:
            average_day_changes = self._get_average_day_change()
            ...
            if percent_day_change > average_day_changes * self.p.c_day_change and (
                ...
                self.log(
                    f'close_price={data.close[0]} | price_take={round(price_take, 2)} | '
                    f'price_stop={round(price_stop, 2)} | '
                    f'percent_day_change={round(percent_day_change * 100, 2)} | '
                    f'average_day_changes={round(average_day_changes * 100, 2)} | '
                    f'percent_change_to_high={round(percent_change_to_high*100, 2)}',
                    data=data
                )

    def _get_average_day_change(self) -> float:
